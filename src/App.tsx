...
const App: React.FC = () => {
  ...
  const logosProgress = [
    ...
    { logo: "https://cdn.simpleicons.org/nextdotjs/white", percentage: 80, name: "Next JS" },
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="flex flex-col items-end gap-4">
                {submitStatus === "success" && (
                  <p className="text-green-500">Pesan berhasil dikirim!</p>
                )}
                {submitStatus === "error" && (
                  <p className="text-red-500 text-right">{errorMessage || "Gagal mengirim pesan. Silakan coba lagi."}</p>
                )}
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="px-8 py-3 bg-primary text-white rounded-md hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
